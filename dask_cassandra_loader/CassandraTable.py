...
class CassandraTable():
    ...
    def load_metadata(self, cassandra_session):
        ...
        self.partition_cols = [f.name for f in cassandra_session.cluster.metadata.keyspaces[self.keyspace].tables[
                                                   self.name].partition_key[:]]

        # load partition keys
        sql_query = sql.select([text(f) for f in self.partition_cols]).distinct().select_from(text(self.name))
        future = cassandra_session.execute_async(str(sql_query))
    ...
    def load_data(self, cassandra_session, caLoadingQuery):
        futures = []

        if self.cols is None:
            self.load_metadata(self.name, cassandra_session)
        ...
        for key_values in partition_keys:
            ...
            future = dask.delayed(self.read_data_)(query, cassandra_session.cluster.contact_points, self.keyspace)
